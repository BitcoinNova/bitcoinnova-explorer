...
function checkTxn(transactionResponse, privateKey, address, privateKeyType) {
    ...
    } else if (address.length !== 95 || (addrHex.slice(-s) !== cn_fast_hash(addrHex.slice(0, -s)).slice(0, s))) {
        results.error = "Bad address";
    } else if (privateKeyType === 'view' && addrHex.slice(m-6, e-6) !== sec_key_to_pub(privateKey)) {
